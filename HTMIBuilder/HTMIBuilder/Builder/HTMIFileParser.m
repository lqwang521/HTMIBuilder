...
+ (void)parser_rw:(NSString *)path code:(NSString *)code filename:(NSString *)filename header:(NSString *)header parameter:(NSMutableArray *)parameter {
    ...
    NSString * template =  [HTMIFileParser parser_r:filename_r code:[code lowercaseString]];
    [[HTMIFileParser replaceThougth:template parameter:parameter] writeToFile:filename_w atomically:YES encoding:NSUTF8StringEncoding error:nil];
}
...
+ (NSString *)replaceThougth:(NSString *)templete parameter:(NSMutableArray *)parameter {
    ...
    if (parameter.count) {
        [HTMIFileParser replaceThougth:temp parameter:parameter];
    } else {
        code = temp;
    }
    return code;
